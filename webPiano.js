...
class webPiano extends HTMLElement {
    constructor (){
        ...
        this.activeNotes = new Proxy(this._activeNotes, {
            set: (target, key, value) => {
              target[key] = value;
              this.dispatchEvent(new CustomEvent('active-notes-changed', { detail: target }));
              return true;
            }
          });
    }
    ...
    _handleKeyDown(event){
        ...
        if (this._activeNotes.indexOf(noteName) == -1){
    ...
    handleTransition(key){
        ...
        selected.forEach((div) => {
            ...
            this._activeNotes.push(noteName);
        })
    }
    ...
    strikeNote(noteDiv){
        if(!noteDiv.classList.contains('selected')){
            ...
            this.shadowRoot.querySelector('.playingDisplay').innerHTML = this._activeNotes;
        }
    }
    ...
    releaseNote(noteDiv){
        if(noteDiv.classList.contains('selected')){
            ...
            const noteName = noteDiv.getAttribute("note");
            var index = this._activeNotes.indexOf(noteName);
